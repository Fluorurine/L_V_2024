...
export class AssistantBackendStack extends cdk.Stack {
	constructor(scope: Construct, id: string, props?: cdk.StackProps) {
		...
		// -----------------------------------------------------------------------
		// Add an PostgreSQL database with PGvector for semantic search. Free Tier
		// Create an  PostgreSQL database, to serve as the semantic search
		...
		// Fix by Truong
		// Generate credentials for the RDS instance and store them in Secrets Manager
		...
		const AgentDB = new rds.DatabaseInstance(this, "AgentDB", {
			engine: rds.DatabaseInstanceEngine.postgres({
				version: rds.PostgresEngineVersion.VER_15_4, // Specify PostgreSQL version 16.3
			}),
			...
			"sageMakerPostgresDBAccessIAMPolicy",
			{
				statements: [
					...
					new iam.PolicyStatement({
						...
						resources: [agent_executor_lambda.functionArn],
					}),
				],
			}
		);
		...
		const cognito_authorizer = new apigateway.CognitoUserPoolsAuthorizer(
			this,
			"ChatAuthorizer",
			{
		...
		const agent_lambda_integration = new apigateway.LambdaIntegration(
